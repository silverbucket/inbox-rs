...
| **WebFinger + OAuth endpoints** | Connect time | `GET https://<your-address-host>/.well-known/webfinger`, then the OAuth authorize redirect. The lookup goes to the domain in your user address, but the storage and authorization hrefs it returns may point at entirely different hosts — the spec permits delegating either. |
| **A Sockethub relay** | Link enrichment (outside the extension), CalDAV | Defaults to `sockethub.silverbucket.net`, which the Inbox RS author operates. Sees the URL of each link you enrich, and — when you publish — your calendar credentials and the event being written. Never sees the rest of your inbox. Changeable in settings; see below. |
...
This path covers links captured *without* the browser extension — the web app,
the quick-capture PWA, the share sheet. The extension doesn't use it at all: its
content script is already running in the page, so it reads the Open Graph tags
directly and never contacts the relay.

For everything else, a bookmark's title, description, site name and preview
image come from Sockethub's stateless `metadata` platform: one `POST` of an
ActivityStreams `fetch`, answered with an NDJSON line carrying the page's Open
Graph fields. No credentials and no connect step, so each lookup is a plain
...
kept as a URL, so it is fetched from the origin at render time — the extension
is the one path that downloads the image itself and stores it as a file.
