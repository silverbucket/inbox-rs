...
{/if}
<div class="row stack"><div class="row-main"><div class="row-label">Sockethub</div><div class="row-desc">The open, multi-protocol relay behind link previews (its <code>metadata</code> platform) and CalDAV calendar sync (its <code>caldav</code> platform) — it does the fetching so your browser and the sites/servers you connect to never talk directly. Requires a Sockethub 5.0 alpha release or later with HTTP actions enabled.</div></div><div class="row-ctl"><div class="seg"><button type="button" class:on={!sockethubCustom} onclick={useDefaultSockethub}>Default</button><button type="button" class:on={sockethubCustom} onclick={()=>sockethubCustom=true}>My own server</button></div>{#if sockethubCustom}<div class="nested"><label for="sockethub-endpoint">Endpoint</label><input id="sockethub-endpoint" class="field mono" type="url" bind:value={sockethubEndpoint} placeholder="https://sockethub.example.com/sockethub-http" onblur={saveSockethub}/></div>{:else}<div class="nested">Runs at <code>{defaultSockethubHost}</code>, hosted by Inbox RS's author as a free service — see <a href="https://silverbucket.net" target="_blank" rel="noreferrer">silverbucket.net</a>.</div>{/if}</div></div>
</div>
