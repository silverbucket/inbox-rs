<script lang="ts">
  ...
  let { section, value, sub, meta = '', warning = false, expanded = false, onclick }: {
    section: SettingsSection; value: string; sub: string; meta?: string; warning?: boolean; expanded?: boolean; onclick: () => void;
  } = $props();
</script>
<button type="button" class="tile" class:wide={section.wide} class:warn={warning}
  aria-label={`${section.title} — ${value}${meta ? `, ${meta}` : ''}${sub ? `, ${sub}` : ''}`}
