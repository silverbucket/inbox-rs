...
test('warm offline still renders shell', async ({ connectedPage, webOrigin }) => {
  ...
  await expect(connectedPage.getByRole('button', { name: 'Inbox' }).first()).toBeVisible();
...
test('cold offline load is blocked without service worker', async ({ page, webOrigin }) => {
